...
<template>
    <ion-list class="ion-list">
      <ion-item v-for="order in filteredOrders" :key="order.id">
        ...
        <ion-icon slot="end" :icon="ellipsisHorizontal" @click="showActions(order)">
  ...
  const showActions = (order:string|number|undefined, event: null) => {
  ...
  const reorderItem = (order: { id:string|number|undefined ; }) => {
    
    orderstore.reorderOrder(order.id);
  };
  
  const sendMessage = (order: { id: string|number|undefined; }) => {
    
    orderstore.sendMessageToSupplier(order.id);
  };
  
  const editOrder = (order: { id:string|number|undefined;  }) => {
    
    orderstore.editOrderDetails(order.id);
  };
  
  const deleteOrder = (order: { id: string|number|undefined; }) => {
